...
RMU_LABELS = {
    ...
    "inventory_issues": "设备数量问题",
    "db_integrity_issues": "数据库字段问题",
}
    ...
    for report in reports:
        ...
        for rmu in report.get("rmu_results", []):
            ...
            base = {
                ...
                "device_count": len([d for d in rmu.get("device_rows", []) if d.get("xml_id")]),
                ...
                "association_block_reasons": "; ".join(rmu.get("association_block_reasons", [])),
                "inventory_issues": "; ".join(rmu.get("inventory_issues", [])),
                "db_integrity_issues": "; ".join(rmu.get("db_integrity_issues", [])),
            }
...
def flatten_device_rows(reports):
    ...
    for report in reports:
        file_name = report.get("file_name", "")
        for rmu in report.get("rmu_results", []):
            for device in rmu.get("device_rows", []):
                row = dict(device)
                row.pop("x", None)
                row.pop("y", None)
...
<main>
  ...
  <div class="card">
    <h2>设备明细</h2><p>每个设备均展示是否已关联、当前 KeyID 反解结果、当前模型所属环网柜 ID、是否关联正确，以及是否需要回写。</p>
    {device_table}
  </div>
</main>
</body>
