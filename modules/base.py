...
class ModelModule(ABC):
    ...
    def preview_association(self, db, files, settings, log_callback, progress_callback=None):
        raise NotImplementedError(f"{self.display_name}: association preview is not implemented yet.")

    def apply_association(self, db, files, settings, preview_data, log_callback):
        raise NotImplementedError(f"{self.display_name}: model write-back is not implemented yet.")
