...
ZH_TO_EN.update({
    ...
    "RMU 环网柜只有在矩形框内同时包含 CBreakerDis、BusDis、ZhaiWaiJieDiDaoZha 时才识别。开关设备名称固定使用环网柜内图上文字；环网柜名称必须由用户指定上方、下方、左侧或右侧方向。设备命名规则不读取三类设备 XML 的 p_NameString：CBreakerDis 使用图上名称，接地刀闸使用开关名+D，BusDis 固定使用 BUS。": "An RMU is recognized only when its rectangle contains CBreakerDis, BusDis, and ZhaiWaiJieDiDaoZha. Device naming uses graphical text inside the RMU, and the operator must explicitly select the RMU-name direction: top, bottom, left, or right. XML p_NameString is not used for these three device types: CBreakerDis uses the graphical name, grounding switches use breaker name + D, and BusDis always uses BUS.",
