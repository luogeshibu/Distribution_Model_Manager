...
ZH_TO_EN.update({
    ...
    "RMU 环网柜通过 G 文件结构自动识别。开关设备名称可选择使用 环网柜内部、紧邻 CBreakerDis 的图上文字。设备命名规则固定使用图上文字，不再读取三类设备 XML 的 p_NameString：CBreakerDis 使用图上名称，接地刀闸使用开关名+D，BusDis 固定使用 BUS。":
        "RMUs are recognized automatically from G-file structure. Device naming uses graphical text inside the RMU near CBreakerDis. XML p_NameString is not used for these three device types: CBreakerDis uses the graphical name, grounding switches use breaker name + D, and BusDis always uses BUS.",
