...
from app_config import (
    ...
    DEFAULT_DB_CONFIG, HISTORY_RETENTION_DAYS,
)
from module_registry import get_model_modules
...
DEFAULT_CONFIG_BUNDLED = RESOURCE_DIR / "config.json"
LOGO_PATH = RESOURCE_DIR / "assets" / "app_logo.png"
ICON_PATH = RESOURCE_DIR / "assets" / "app_logo.ico"


def get_workspace_dir(input_value: str) -> Path:
    path = Path(input_value)
    return path.parent if path.is_file() else path


def cleanup_workspace_reports(workspace_dir: Path):
    report_root = workspace_dir / "Distribution_Model_Reports"
...
def load_config():
    ...
    source = CONFIG_PATH if CONFIG_PATH.exists() else DEFAULT_CONFIG_BUNDLED
    if source.exists():
        try:
...
class JobWorker(QThread):
    ...
    def __init__(self, cfg, module, operation, settings, files):
        super().__init__()
        self.cfg = cfg
        self.module = module
    ...
    def run(self):
        db = None
        try:
            workspace_dir = get_workspace_dir(self.cfg["input_path"])
            cleanup_workspace_reports(workspace_dir)
            ...
            self.cfg["last_report_dir"] = str(report_dir)
            save_config(self.cfg)

            self.progress.emit(100, "任务处理完成")
            self.completed.emit(str(report_dir), summary, rules, preview_data, artifacts)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.current_report_dir = self.cfg.get("last_report_dir", "")
        self.worker = None
        ...
        self.log("在没有明确预览和人工确认之前，程序不会修改 G 文件。")
    ...
    def _build_workspace_page(self):
        ...
        for module_id, module in self.modules.items():
            widget = module.create_settings_widget(self.module_stack, self.cfg)
        ...
        self.open_report_dir_btn = QPushButton("打开报告目录")
        ...
        self.open_report_dir_btn.clicked.connect(lambda: self.open_artifact("report_dir"))
    ...
    def _build_settings_page(self):
        ...
        text = QLabel(
            f"• 当前工作目录下自动生成的报告保留 {HISTORY_RETENTION_DAYS} 天\n"
    ...
    def _build_help_page(self):
        ...
        assoc_text = QLabel(
            ...
            "真正执行模型关联时，程序会先重新检查数据库及预览有效性，然后备份原 G 文件。\n\n"
    ...
    def db_log(self, text):
        if hasattr(self, "db_log_edit"):
            self.db_log_edit.appendPlainText(str(text))
    ...
    def start_job(self):
        try:
            ...
            settings = module.collect_settings(
    ...
    def apply_association(self):
        ...
        try:
            ...
            module_id = self.module_combo.currentData()
            module = self.modules[module_id]
            settings = module.collect_settings(self.module_widgets[module_id])
            ...
            results = module.apply_association(
                db, files, settings, self.current_preview, self.log
            )
            db.close()

            total = sum(int(r.get("applied_count", 0)) for r in results)
            backups = "\n".join(str(r.get("backup", "")) for r in results)
            self.current_preview = None
            self.apply_btn.setEnabled(False)
            self.progress_bar.setValue(100)
            ...
            QMessageBox.information(
                self, "模型关联完成",
                f"模型关联回写完成，共修改 {total} 个设备图元。\n\n备份文件：\n{backups}"
            )
            self.log(f"模型关联回写完成：共修改 {total} 个设备图元。")
        except Exception as exc:
