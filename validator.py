...
from g_parser import GParser, RmuFrame, GObject
...
class RmuValidator:
    ...
    def validate_file(self, g_path: str | Path, positions: Sequence[str], progress_callback=None) -> Dict[str, Any]:
        ...
        for index, frame in enumerate(frames, start=1):
            ...
            # Strict inventory parity: G count must exactly equal DB count.
            for tag in self.device_rules:
                g_count = len(elements_by_tag[tag])
                db_count = len(db_sets.get(tag, {}).get("rows", []))
                rmu_result["g_inventory"][tag] = g_count
                if g_count != db_count:
                    issue = f"DEVICE_COUNT_MISMATCH:{tag}:G={g_count}:DB={db_count}"
                    ...
                    if db_count > g_count:
                        ...
                        rmu_result["association_block_reasons"].append(
                            "环网柜数据库中的设备数量少于G文件中的设备数量，请检查数据库模型。"
                        )

            # Database-field integrity is checked across the complete RMU device set,
            # not only records that happen to match a G element.
            for tag, db_set in db_sets.items():
                code_counts = defaultdict(int)
                for dev in db_set.get("rows", []):
                    did = dev.get("id", "")
                    code = norm(dev.get("code"))
                    name = norm(dev.get("name"))
                    if code:
                        code_counts[code] += 1
                    else:
                        issue = f"DB_CODE_EMPTY:{tag}:DEVICE_ID={did}"
                        rmu_result["db_integrity_issues"].append(issue)
                        rmu_result["association_block_reasons"].append(issue)

                for code, count in code_counts.items():
                    if count > 1:
                        issue = f"DB_CODE_DUPLICATE:{tag}:CODE={code}:COUNT={count}"
                        rmu_result["db_integrity_issues"].append(issue)
                        rmu_result["association_block_reasons"].append(issue)
            ...
            matched_db_ids = defaultdict(set)
            ...
            for elem in breakers:
                ...
                did = int_or_none(row.get("db_device_id"))
                if did is not None:
                    matched_db_ids[elem.tag].add(did)
                rmu_result["device_rows"].append(row)
            ...
            # Make every extra/unmatched DB record visible as a blocking FAIL row.
            for tag, db_set in db_sets.items():
                for dev in db_set.get("rows", []):
                    did = int_or_none(dev.get("id"))
                    if did is None or did in matched_db_ids[tag]:
                        continue
                    fake = {
                        "rmu_name": rmu_result["rmu_name"],
                        "rmu_id": rmu_id,
                        "object_type": tag,
                        "xml_id": "",
                        "p_name_string": "",
                        "graphical_name": "",
                        "selected_name_source": "DATABASE_INVENTORY",
                        "selected_device_name": "",
                        "paired_breaker_name": "",
                        "table_id": db_set["table_id"],
                        "table_name": db_set["table_name"],
                        "configured_domain": db_set["domain"],
                        "match_mode": self.device_rules[tag].get("match_mode", ""),
                        "db_match_count": 1,
                        "db_device_id": dev.get("id", ""),
                        "db_code": norm(dev.get("code")),
                        "db_name": norm(dev.get("name")),
                        ...
                        "model_linked": "",
                        "model_link_correct": "",
                        "model_link_status": "数据库存在额外设备，无对应G图元",
                        "association_action": "禁止关联",
                        "writeback_needed": "NO",
                        "association_ready": "NO",
                        "status": "FAIL",
                        "reason": "DB_EXTRA_OR_UNMATCHED_DEVICE",
                    }
                    rmu_result["device_rows"].append(fake)
